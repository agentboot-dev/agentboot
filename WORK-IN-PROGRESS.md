...
This was extracted from a private production persona system built for NourishIQ (a healthcare platform). That reference system has:
- Multi-level scope hierarchy (org → group → team → repo)
- Composable traits with configurable weights
- PHI guardrail hooks (HIPAA compliance)
- Build/validate/sync pipeline with CI gates
- Distribution to a registered list of repos

AgentBoot is the **generic, open-source, domain-agnostic** version of that system. No healthcare content, no proprietary org structure. MIT licensed. The goal is that this could be used by any engineering org, and the design should never reveal what org it came from.

**Key distinction:** The reference system (private, NourishIQ) is the prior art. AgentBoot is the public abstraction. They must never be linked. Do not reference NourishIQ, Fortael, Christie, or the healthcare domain anywhere in this repo.
...
| License | `LICENSE` | ✅ MIT, Mike Saavedra |
