...
const DEFAULT_TELEMETRY_DIR = path.join(process.env["HOME"] ?? "~", ".agentboot", "telemetry");
...
export function generateModelRecommendations(metrics: PersonaMetrics[]): ModelRecommendation[] {
  ...
  for (const [personaId, personaMetrics] of byPersona) {
    ...
    const avgRephraseRate = personaMetrics.reduce((s, m) => s + m.rephraseRate, 0) / personaMetrics.length;
...
export function generateHtmlReport(
  ...
  for (let i = 0; i < topN.length; i++) {
    ...
    barsSvg += `<text x="0" y="${y + 20}" font-size="12" fill="#333">${m.personaId}</text>`;
  ...
  for (const rec of recommendations) {
    recsHtml += `<div style="margin:10px 0;padding:10px;background:#f8f9fa;border-radius:4px;">
      <strong>${rec.personaId}</strong>: ${rec.currentModel} -> ${rec.recommendedModel}
      ${rec.projectedSavingsUsd > 0 ? `<span style="color:green;"> (save ~$${rec.projectedSavingsUsd}/mo)</span>` : ""}
      <p style="color:#666;margin:4px 0;">${rec.rationale}</p>
      <p style="color:#999;margin:4px 0;">Risk: ${rec.risk}</p>
    </div>`;
  }
...
<body>
  ...
  ${gaps.length > 0 ? `<h2>Coverage Gaps</h2>${gaps.map(g => `<p class="gap">Warning: ${g.message}</p>`).join("")}` : ""}
