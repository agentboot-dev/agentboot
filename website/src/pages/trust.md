...
| Enforcement is blocking on the official CLI surfaces | Hooks are emitted into `.claude/settings.json`, `.codex/hooks.json`, and `.github/hooks/agentboot.json` from one canonical script set, blocking on exit code 2 on Claude Code and Codex. Two stated caveats for Copilot CLI: command-hooks **fail open on timeout** (a hung or slow hook does not block there), and its exit-2 blocking is **documented platform behavior not yet empirically verified** end-to-end — see the [capability matrix](/docs/platform-capability-matrix) | Read the emitted hook files in a compiled repo; trigger a guarded action in Claude Code or Codex and watch it block. |
...
- **Enforcement depth varies by platform, even within the official tier.** GitHub Copilot CLI's hook ceiling is lower than Claude Code's and Codex's — Copilot command-hook timeouts **fail open** (a hung or slow hook allows the action instead of blocking it), and Copilot's exit-2 blocking is **documented but not yet empirically verified** the way Claude Code's and Codex's is. The community-tier platforms get advisory output and drift checking, with no blocking enforcement at all.
