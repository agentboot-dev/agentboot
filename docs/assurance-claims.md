...
| 3 | Synced manifests are tamper-evident | `tests/hardening-v0.16.test.ts` (executed signature-strip attack, rogue-signer rejection) | ONLY with `sync.signing` enabled AND verification run with `--require-signed --allowed-signers`. An unsigned digest detects accidental corruption only — `verify-manifest` reports this posture explicitly. |
