...
> **GAP:** This file is referenced throughout the design but is currently missing
> from all personas in the codebase. The schema below is inferred from design
> documents and must be formalized.
...
### OQ-01: persona.config.json Schema Formalization

**Status:** Gap.

The `persona.config.json` file is referenced throughout the design documentation but
does not exist for any of the four initial personas. The schema described in Section
4.2 is inferred from multiple design documents. This schema needs to be formally
defined, validated against a JSON Schema, and populated for all personas.

**Impact:** Blocks trait composition in the build pipeline.

---

### OQ-02: Trait Weight Section Extraction

**Status:** Unresolved.

How does the build system extract the "HIGH", "MEDIUM", or "LOW" section from a trait
file? The current trait files (e.g., `critical-thinking.md`) are prose documents. The
compilation phase needs a convention for marking weight-specific content within trait
files. Options:

a) Markdown heading convention: `## HIGH`, `## MEDIUM`, `## LOW` sections
b) Frontmatter with separate content blocks
c) A single prose definition with weight-specific directives inline

This convention needs to be defined and documented.

---

### OQ-03: Cross-Platform Gotchas Frontmatter

**Status:** Ambiguity.

Claude Code rules use `paths:` frontmatter. Copilot instructions use glob-style
`applyTo:` frontmatter. Cursor rules have their own format. The build system needs
to translate between these formats during output generation. The translation rules
need to be formalized.

---

### OQ-04: Plugin Namespace Impact on UX

**Status:** Trade-off.

CC plugins namespace their skills: `/agentboot:review-code` instead of `/review-code`.
This prevents conflicts but adds friction. Should AgentBoot recommend that orgs rename
their marketplace to get shorter namespaces (e.g., `/acme:review-code`)? Or is the
namespace overhead acceptable?

---

### OQ-05: Sync Conflict Resolution

**Status:** Unresolved.

When a developer manually edits a file that AgentBoot manages (e.g., adds content to
`.claude/CLAUDE.md`), the next sync may overwrite their changes. The manifest tracks
hashes to detect this, but the conflict resolution behavior is not defined:

a) Warn and skip the file
b) Merge (with what strategy?)
c) Overwrite with a backup
d) Fail and require manual resolution

---

### OQ-06: repos.json Doesn't Exist

**Status:** Gap.

The `repos.json` file referenced by the sync engine does not exist in the repository.
It needs to be created (can start as `[]`) to unblock the sync pipeline.

---

### OQ-07: Missing Trait Definitions

**Status:** Gap.

Traits `minimal-diff` and `explain-reasoning` are referenced in `agentboot.config.json`
but do not have corresponding files in `core/traits/`. These need to be authored or
the references need to be removed from the config.

---

### OQ-08: Knowledge Frontmatter Schema for Stage 2

**Status:** Unresolved.

The knowledge item frontmatter described in Section 4.7 is proposed but not formalized.
Questions:
- Which fields are required vs. optional?
- Should there be a JSON Schema for knowledge frontmatter?
- How does `agentboot build --index` discover knowledge files (by directory convention
  or by scanning for frontmatter)?

---

### OQ-09: CLI Implementation Language

**Status:** Deferred.

The design recommends starting with TypeScript (reuse existing code) and potentially
migrating to Go for binary distribution. The decision on whether to do a Go rewrite
depends on whether binary size and startup time become real problems.
...
The autonomy model (Advisory > Auto-approve > Autonomous) requires "evidence" for
promotion, but the evidence criteria are not defined:
- What metrics qualify a persona for promotion?
- What is the minimum time at each phase?
- Who approves the promotion (team lead? org admin?)?
- Is demotion possible (and when)?

---

### OQ-11: Domain Layer Conflict Between Domains

**Status:** Partially resolved.

When two domain layers define a trait with the same ID, the build fails. But what
about subtler conflicts -- two domains that give contradictory instructions for the
same code path? The build system can detect ID collisions but not semantic conflicts.

---

### OQ-12: Server-Managed Settings Integration

**Status:** Future.

Anthropic's server-managed settings (no MDM required) are mentioned as an alternative
to endpoint MDM. The integration path for AgentBoot to generate/upload server-managed
settings is not defined. This depends on Anthropic's admin API surface.
...
When the `/review` meta-skill detects conflicting findings from multiple reviewers,
the persona arbitrator resolves the conflict. The criteria for "conflicting findings"
are not defined:
- Same code location with different severity?
- Contradictory recommendations?
- Different findings that are logically incompatible?
...
When a plugin is updated in the marketplace:
- Does `/reload-plugins` pull the latest automatically?
- Can orgs pin to a specific version?
- How are breaking changes handled?
- Is there a rollback mechanism?

---

### OQ-15: Test Infrastructure

**Status:** Gap.

No test files exist in the repository despite vitest being configured. The 6-layer
test pyramid described in the test plan document needs initial implementation,
starting with unit/schema tests for config validation and frontmatter parsing.

---

### OQ-16: Team-Level Config Storage

**Status:** Ambiguity.

Team-level configuration overrides are mentioned but their storage location is unclear.
Are they:
a) Sections within `agentboot.config.json`?
b) Separate files under a `teams/` directory?
c) Derived from `repos.json` team assignments plus group config?

The current design appears to use option (c) -- team identity comes from `repos.json`
and team config inherits from the group. But explicit team-level overrides (beyond
what the group provides) need a defined storage mechanism.
...
Cowork plugins use the same format as CC plugins but surface in a GUI with forms.
The question is which AgentBoot skills make sense for non-engineers and how the
form-based input maps to persona invocation.
...
Stage 3 vector search requires embeddings. Which embedding model?
- Anthropic embeddings API (native if using Claude)
- OpenAI embeddings (widely used, low cost)
- Local models (no API dependency, higher setup cost)

The choice affects cost, latency, and privacy (whether code snippets leave the
machine during embedding).

---

### OQ-19: dist/ Directory Lifecycle

**Status:** Gap.

The `dist/` directory has never been produced by a successful end-to-end pipeline
run. The output directory structure described in this document is designed but not
verified. The first successful `agentboot build` will validate (or require revisions
to) the output structure.

---

### OQ-20: Hook Script Portability

**Status:** Unresolved.

Hook scripts are currently designed as bash scripts. This works on macOS and Linux
but not on Windows without WSL. For organizations with Windows developers:
- Should hook scripts be portable (Node.js scripts)?
- Should the CLI generate platform-specific hook scripts?
- Should hooks be TypeScript and bundled with the CLI?
